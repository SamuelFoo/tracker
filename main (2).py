...
def evaluateCircleMatch(edges, centreX, centreY, radius):
    ...
    circle = cv2.circle(
        mask, (int(centreX), int(centreY)), int(radius), (255, 255, 255), 1
    )  # Change remove "circle ="?
    ballEdge = edges & mask
...
def detectCircleMorph(frameSaturation, ballRadiusEst):
    ...
    _, thresh = cv2.threshold(
        frameSaturation, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )
    ...
    if np.abs(ellipseAxes[0] / 2 - ballRadiusEst) < np.abs(
        ellipseAxes[1] / 2 - ballRadiusEst
    ):
...
def detectCircleRANSAC(maxIterations, edgesFrame):
    ...
    for i in range(maxIterations):
        ...
        circle = cv2.circle(
            mask, (int(centerx), int(centery)), int(radius), (255, 255, 255), 1
        )
...
def getTaitBryanAngles(r1, r2, theta1, theta2, r1_dt, r2_dt, theta1_dt, theta2_dt):
...
if __name__ == "__main__":
    ...
    for title in files:
        ...
        h = len(img_resized)
        w = len(img_resized[0])
        img_trunc = img_resized[int(9 * h / 18) : int(h * 30 / 48)][
            :, int(0 * w / 9) : int(4 * w / 9)
        ]
        ...
        cv2.circle(
            img_trunc,
            (int(bestCenterX), int(bestCenterY)),
            int(bestR),
            (255, 255, 255),
            1,
        )
        ...
        ydisp = (
            bestCenterY - yfunnel
        ) * calibScale  # Note, y axis of cv2 is flipped. Origin at top left hand corner of image

        cv2.circle(img_trunc, (int(bestCenterX), int(yfunnel)), 2, (0, 255, 0), -1)
        cv2.imwrite(
            "./" + title.split(".")[0] + "track " + str(ydisp) + ".JPG", img_trunc
        )
